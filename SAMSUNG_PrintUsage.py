...
#rdr = csv.DictReader(f)
    ...
    for i in rdr:
        ...
        while DeptandUserName:
            ...
            DeptName = str(DeptandUserName[2])
            UserName = str(DeptandUserName[1])
            #print(DeptName + UserName)
            DeptandUserName = db.dbFetchOne()
            
            db.dbInsertIndividual(datetime.today().strftime("%Y%m%d"),i[2],DeptName,UserName,monoPrintUse,colorPrintUse,monoCopyUse,colorCopyUse,Di,datetime.today())
            dictWrite.writerow({'부서':DeptName,'성명': UserName,'사번' : i[2], '컬러출력' : colorPrintUse,
